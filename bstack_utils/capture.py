# coding: UTF-8
import sys
bstack1_opy_ = sys.version_info [0] == 2
bstack11l1ll1_opy_ = 2048
bstack1l1l1ll_opy_ = 7
def bstack11l1l11_opy_ (bstack111l1ll_opy_):
    global bstack1l1lll1_opy_
    bstack1l1l11_opy_ = ord (bstack111l1ll_opy_ [-1])
    bstack111l1l1_opy_ = bstack111l1ll_opy_ [:-1]
    bstack111ll_opy_ = bstack1l1l11_opy_ % len (bstack111l1l1_opy_)
    bstack11l11l1_opy_ = bstack111l1l1_opy_ [:bstack111ll_opy_] + bstack111l1l1_opy_ [bstack111ll_opy_:]
    if bstack1_opy_:
        bstack1111l1l_opy_ = unicode () .join ([unichr (ord (char) - bstack11l1ll1_opy_ - (bstack1l111ll_opy_ + bstack1l1l11_opy_) % bstack1l1l1ll_opy_) for bstack1l111ll_opy_, char in enumerate (bstack11l11l1_opy_)])
    else:
        bstack1111l1l_opy_ = str () .join ([chr (ord (char) - bstack11l1ll1_opy_ - (bstack1l111ll_opy_ + bstack1l1l11_opy_) % bstack1l1l1ll_opy_) for bstack1l111ll_opy_, char in enumerate (bstack11l11l1_opy_)])
    return eval (bstack1111l1l_opy_)
import builtins
import logging
class bstack1l11l111_opy_:
    def __init__(self, handler):
        self._111ll1lll11_opy_ = builtins.print
        self.handler = handler
        self._started = False
        logging.basicConfig(level=logging.INFO)
        self.logger = logging.getLogger(__name__)
        self._111lll11111_opy_ = {
            level: getattr(self.logger, level)
            for level in [bstack11l1l11_opy_ (u"ࠨ࡫ࡱࡪࡴ࠭᮴"), bstack11l1l11_opy_ (u"ࠩࡧࡩࡧࡻࡧࠨ᮵"), bstack11l1l11_opy_ (u"ࠪࡻࡦࡸ࡮ࡪࡰࡪࠫ᮶"), bstack11l1l11_opy_ (u"ࠫࡪࡸࡲࡰࡴࠪ᮷")]
        }
    def start(self):
        if self._started:
            return
        self._started = True
        builtins.print = self._111ll1lll1l_opy_
        self._111ll1llll1_opy_()
    def _111ll1lll1l_opy_(self, *args, **kwargs):
        self._111ll1lll11_opy_(*args, **kwargs)
        message = bstack11l1l11_opy_ (u"ࠬࠦࠧ᮸").join(map(str, args)) + bstack11l1l11_opy_ (u"࠭࡜࡯ࠩ᮹")
        self._log_message(bstack11l1l11_opy_ (u"ࠧࡊࡐࡉࡓࠬᮺ"), message)
    def _log_message(self, level, msg, *args, **kwargs):
        if self.handler:
            self.handler({bstack11l1l11_opy_ (u"ࠨ࡮ࡨࡺࡪࡲࠧᮻ"): level, bstack11l1l11_opy_ (u"ࠩࡰࡩࡸࡹࡡࡨࡧࠪᮼ"): msg})
    def _111ll1llll1_opy_(self):
        for level, bstack111ll1lllll_opy_ in self._111lll11111_opy_.items():
            setattr(logging, level, self._111lll1111l_opy_(level, bstack111ll1lllll_opy_))
    def _111lll1111l_opy_(self, level, bstack111ll1lllll_opy_):
        def wrapper(msg, *args, **kwargs):
            bstack111ll1lllll_opy_(msg, *args, **kwargs)
            self._log_message(level.upper(), msg)
        return wrapper
    def reset(self):
        if not self._started:
            return
        self._started = False
        builtins.print = self._111ll1lll11_opy_
        for level, bstack111ll1lllll_opy_ in self._111lll11111_opy_.items():
            setattr(logging, level, bstack111ll1lllll_opy_)