# coding: UTF-8
import sys
bstack1lll1_opy_ = sys.version_info [0] == 2
bstack11l11_opy_ = 2048
bstack1_opy_ = 7
def bstack1l1_opy_ (bstack1llllll_opy_):
    global bstack1l11111_opy_
    bstack1l111l_opy_ = ord (bstack1llllll_opy_ [-1])
    bstack11l1ll1_opy_ = bstack1llllll_opy_ [:-1]
    bstack11l1l1l_opy_ = bstack1l111l_opy_ % len (bstack11l1ll1_opy_)
    bstack11111l1_opy_ = bstack11l1ll1_opy_ [:bstack11l1l1l_opy_] + bstack11l1ll1_opy_ [bstack11l1l1l_opy_:]
    if bstack1lll1_opy_:
        bstack1lllll1_opy_ = unicode () .join ([unichr (ord (char) - bstack11l11_opy_ - (bstack1l1ll11_opy_ + bstack1l111l_opy_) % bstack1_opy_) for bstack1l1ll11_opy_, char in enumerate (bstack11111l1_opy_)])
    else:
        bstack1lllll1_opy_ = str () .join ([chr (ord (char) - bstack11l11_opy_ - (bstack1l1ll11_opy_ + bstack1l111l_opy_) % bstack1_opy_) for bstack1l1ll11_opy_, char in enumerate (bstack11111l1_opy_)])
    return eval (bstack1lllll1_opy_)
import threading
from collections import deque
from bstack_utils.constants import *
class bstack1lll1l11l1_opy_:
    def __init__(self):
        self._111ll11l11l_opy_ = deque()
        self._111ll111lll_opy_ = {}
        self._111ll11l111_opy_ = False
        self._lock = threading.RLock()
    def bstack111ll1l11ll_opy_(self, test_name, bstack111ll11l1ll_opy_):
        with self._lock:
            bstack111ll11lll1_opy_ = self._111ll111lll_opy_.get(test_name, {})
            return bstack111ll11lll1_opy_.get(bstack111ll11l1ll_opy_, 0)
    def bstack111ll1l11l1_opy_(self, test_name, bstack111ll11l1ll_opy_):
        with self._lock:
            bstack111ll1l111l_opy_ = self.bstack111ll1l11ll_opy_(test_name, bstack111ll11l1ll_opy_)
            self.bstack111ll11ll1l_opy_(test_name, bstack111ll11l1ll_opy_)
            return bstack111ll1l111l_opy_
    def bstack111ll11ll1l_opy_(self, test_name, bstack111ll11l1ll_opy_):
        with self._lock:
            if test_name not in self._111ll111lll_opy_:
                self._111ll111lll_opy_[test_name] = {}
            bstack111ll11lll1_opy_ = self._111ll111lll_opy_[test_name]
            bstack111ll1l111l_opy_ = bstack111ll11lll1_opy_.get(bstack111ll11l1ll_opy_, 0)
            bstack111ll11lll1_opy_[bstack111ll11l1ll_opy_] = bstack111ll1l111l_opy_ + 1
    def bstack1l111ll1ll_opy_(self, bstack111ll11ll11_opy_, bstack111ll1l1111_opy_):
        bstack111ll11l1l1_opy_ = self.bstack111ll1l11l1_opy_(bstack111ll11ll11_opy_, bstack111ll1l1111_opy_)
        event_name = bstack11l1l111l11_opy_[bstack111ll1l1111_opy_]
        bstack11lllll1111_opy_ = bstack1l1_opy_ (u"ࠨࡻࡾ࠯ࡾࢁ࠲ࢁࡽࠣᯜ").format(bstack111ll11ll11_opy_, event_name, bstack111ll11l1l1_opy_)
        with self._lock:
            self._111ll11l11l_opy_.append(bstack11lllll1111_opy_)
    def bstack1111l11l11_opy_(self):
        with self._lock:
            return len(self._111ll11l11l_opy_) == 0
    def bstack1111l1l11l_opy_(self):
        with self._lock:
            if self._111ll11l11l_opy_:
                bstack111ll11llll_opy_ = self._111ll11l11l_opy_.popleft()
                return bstack111ll11llll_opy_
            return None
    def capturing(self):
        with self._lock:
            return self._111ll11l111_opy_
    def bstack11lll1ll1_opy_(self):
        with self._lock:
            self._111ll11l111_opy_ = True
    def bstack11ll11l111_opy_(self):
        with self._lock:
            self._111ll11l111_opy_ = False