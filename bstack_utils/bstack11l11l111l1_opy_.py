...
class bstack111ll1111ll_opy_:
    def __init__(self, handler, bstack111l1lllll1_opy_=bstack111ll111l1l_opy_, bstack111ll1111l1_opy_=bstack111ll11111l_opy_):
        self.queue = []
    ...
    def bstack111l1llllll_opy_(self):
        try:
            ...
            logger.debug(bstack11l111_opy_ (u"ࠫࡠࡹࡴࡰࡲࡢࡸ࡮ࡳࡥࡳ࡟ࠣࡉࡽࡩࡥࡱࡶ࡬ࡳࡳࡀࠠࠨᯨ") + (str(e) or bstack11l111_opy_ (u"ࠧࡋࡸࡤࡧࡳࡸ࡮ࡵ࡮ࠡࡥࡲࡹࡱࡪࠠ࡯ࡱࡷࠤࡧ࡫ࠠࡤࡱࡱࡺࡪࡸࡴࡦࡦࠣࡸࡴࠦࡳࡵࡴ࡬ࡲ࡬ࠨᯩ")))
        finally:
            self.timer = None
    ...
    def add(self, event):
        with self.lock:
            ...
            if len(self.queue) >= self.bstack111l1lllll1_opy_:
                threading.Thread(target=self.bstack111l1llll1l_opy_).start()
    def bstack111l1llll1l_opy_(self, source = bstack11l111_opy_ (u"࠭ࠧᯪ")):
        with self.lock:
            if not self.queue:
        ...
        self.handler(data)
        if source != bstack11l111_opy_ (u"ࠧࡴࡪࡸࡸࡩࡵࡷ࡯ࠩᯫ"):
            self.bstack111l1llll11_opy_()
    def shutdown(self):
        self.bstack111l1llllll_opy_()
        while self.queue:
            self.bstack111l1llll1l_opy_(source=bstack11l111_opy_ (u"ࠨࡵ࡫ࡹࡹࡪ࡯ࡸࡰࠪᯬ"))