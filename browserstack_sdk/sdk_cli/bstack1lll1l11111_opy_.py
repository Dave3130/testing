...
class bstack1lll11lllll_opy_:
    timeout: int
    bstack1l1ll1ll1ll_opy_: Union[None, Callable]
    bstack1l1ll1llll1_opy_: Union[None, Callable]
    def __init__(self, timeout=1, bstack1l1ll1lll11_opy_=1, bstack1l1ll1ll1ll_opy_=None, bstack1l1ll1llll1_opy_=None):
    ...
    def enqueue(self, job: Callable):
        if not callable(job):
            raise ValueError(bstack1ll11_opy_ (u"ࠥ࡭ࡳࡼࡡ࡭࡫ࡧࠤ࡯ࡵࡢ࠻ࠢࠥዅ") + type(job))
        self.queue.put(job)
    def start(self):
        if self.threads:
            return
        self.threads = [threading.Thread(target=self.worker, daemon=True) for _ in range(self.bstack1l1ll1lll11_opy_)]
    ...
    def stop(self):
        ...
        self.threads.clear()
    def worker(self):
        while not self.bstack1l1ll1lll1l_opy_.is_set():
            try:
                ...
                if callable(self.bstack1l1ll1llll1_opy_):
                    self.bstack1l1ll1llll1_opy_(e)