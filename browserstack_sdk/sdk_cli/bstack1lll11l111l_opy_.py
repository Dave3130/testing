...
class bstack1lll111llll_opy_:
    ...
    bstack1l1ll11ll11_opy_: Union[None, Callable]
    def __init__(self, timeout=1, bstack1l1ll11ll1l_opy_=1, bstack1l1ll11l1ll_opy_=None, bstack1l1ll11ll11_opy_=None):
    ...
    def enqueue(self, job: Callable):
        if not callable(job):
            raise ValueError(bstack11111_opy_ (u"ࠨࡩ࡯ࡸࡤࡰ࡮ࡪࠠ࡫ࡱࡥ࠾ࠥࠨዹ") + type(job))
        self.queue.put(job)
    def start(self):
        if self.threads:
            return
        self.threads = [threading.Thread(target=self.worker, daemon=True) for _ in range(self.bstack1l1ll11ll1l_opy_)]
    ...
    def stop(self):
        ...
        self.threads.clear()
    def worker(self):
        while not self.bstack1l1ll11lll1_opy_.is_set():
            try:
                ...
                if callable(self.bstack1l1ll11ll11_opy_):
                    self.bstack1l1ll11ll11_opy_(e)